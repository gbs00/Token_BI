...
控制台页面由 `scripts/control_panel.py` 读取 `scripts/control_panel.html` 输出。

当前视觉方向：

- 深色桌面 App 风格
- 顶部大标题、说明、服务状态和服务主按钮
- 当前账号、数据源、看板端口和最近同步组成摘要区
- 快捷操作、服务状态、副屏入口和最近日志组成主控制区；不再维护旧 checklist/compact guide 数据
- 每 5 秒从主服务读取一次 `runtime-status`，不在正常轮询中附带诊断请求；诊断 API 保留供排障调用
- 日志摘要读取末尾最多 64 KiB 并显示最近 20 行，不全文加载持续增长的日志
