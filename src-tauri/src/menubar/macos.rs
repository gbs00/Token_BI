...
pub(super) fn position(app: &tauri::AppHandle, window: &WebviewWindow) -> Result<(), String> {
    ...
    native.setFrame_display(panel_frame(tray, area), false);
    native.invalidateShadow();
    Ok(())
}

fn panel_frame(tray: Option<NSRect>, area: NSRect) -> NSRect {
    let margin = SCREEN_MARGIN
        .min(area.size.width / 2.0)
        .min(area.size.height / 2.0);
    let width = PANEL_WIDTH.min((area.size.width - margin * 2.0).max(1.0));
    let height = PANEL_HEIGHT.min((area.size.height - margin * 2.0).max(1.0));
...
mod tests {
    ...
    fn panel_frame_is_in_logical_points_below_the_tray() {
        ...
        assert_eq!(panel_frame(Some(tray), area), expected);
    }

    #[test]
    fn negative_origin_and_dock_insets_are_respected() {
        let area = rect(-1870.0, -120.0, 1814.0, 1052.0);
        let frame = panel_frame(Some(rect(-64.0, 932.0, 24.0, 28.0)), area);
    ...
    fn missing_tray_and_small_work_area_stay_on_screen() {
        assert_eq!(
            panel_frame(None, rect(-320.0, 24.0, 320.0, 400.0)),
