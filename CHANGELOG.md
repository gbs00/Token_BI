...
日期：2026-09-25。用户授权完成本地验证后正式发布；本机安装、自动化验证及正式 WKWebView 组件真人登录验收已完成。
...
- 共享一套 Python 运行库、裁剪发布符号表并排除无用命令行依赖：首轮 1.2.6 App 54.74 MB → 40.14 MB，DMG 22.99 MB → 17.83 MB；两个服务仍独立运行。新增单运行库门禁与旧目录清理的更新安装验证，详见[精简验证记录](docs/releases/v1.2.6-shared-runtime.md)。随后按用户要求替换本机，账号保留、真实 OAuth、额度面板、看板及二维码检查通过，尚未发布。
